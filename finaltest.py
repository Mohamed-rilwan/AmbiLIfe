...
for index,j in enumerate(ambulance_loc):
    ...
    m = folium.Map(location=[12.916469, 77.618870], tiles="CartoDBPositron", zoom_start=17)
    folium.Marker([data.iloc[index]['lon'], data.iloc[index]['lat']], bindLabel("Ambulance live location"),
                  icon=folium.Icon(
                      icon_color='#FFFFFF',
    ...
    if distances[0][-1] > 0.4:
        ...
        folium.Marker(location=signalA, bindLabel("Silkboard traffic signal"), icon=folium.Icon(
            icon_color='#FF0000',
            icon="fa-circle", prefix='fa')).add_to(m)
        folium.Marker(location=signalB, bindLabel("Silkboard traffic signal"), icon=folium.Icon(
            icon_color='#00FF00',
            icon="fa-circle", prefix='fa')).add_to(m)
        folium.Marker(location=signalC, bindLabel("Silkboard traffic signal"), icon=folium.Icon(
            icon_color='#FF0000',
            icon="fa-circle", prefix='fa')).add_to(m)
        m.save('index.html')
